...
def function_test_steps(stop_event):
    while True:
...
def process_csv_to_webclient(dict_credentials, stop_event=None):
    ...
    driver = launch_navigator(dict_credentials)
    ...
    if dict_credentials['current_url'] == 'https://mjrealestate.3cx.us/webclient/#/people':
        # function_test_steps(stop_event)
...
def click_menucontacts():
    menucontacts = driver.find_element(By.ID, 'menuContacts')
    menucontacts.click()
    time.sleep(0.3)

def wait_list_contacts():
    flag_continue = True
    while flag_continue:
        try:
            # find list of phone contacts
...
def confirm_delete_contact():
    flag_wait_confirmation = True # flag to search emerged windows to confirm delete
...
def click_delete_option(id_contact):
    show_more = phone_contacts[id_contact].find_element(By.ID, 'showMoreBtnTile')# click on button show more
    show_more.click()
    time.sleep(0.25)    
    drop_down = driver.find_element(By.CLASS_NAME, 'dropdown-menu.ng-star-inserted') # find dropdown
    options = drop_down.find_elements(By.CLASS_NAME, 'ng-star-inserted') # get options from dropdown
    time.sleep(0.25)
    flag_delete = False
    for option in options:        
        if 'Delete' in option.text: # check options and click when appear delete
            option.click()
            time.sleep(0.25)
            flag_delete = True
            break
...
def possible_emerged_erro(max_try = 3):
    count = 0
    wait_flag = True
    while wait_flag:
        try:
            errormsg = driver.find_element(By.CLASS_NAME, 'modal-body')
            if "Contact is not found" in errormsg.text: # in case of apper the error "contact is not found" click on ok
                button_ok = driver.find_element(By.CLASS_NAME, 'btn.btn-primary.ng-star-inserted')
                button_ok.click()
                wait_flag = False
        except:
            ...
            time.sleep(0.35)
            count +=1
            if count == max_try: # in case that not appear the error count reach max count of 3 and continue
                wait_flag = False
...
def delete_contacts():
    ...
    click_menucontacts()    
    phone_contacts = wait_list_contacts()            
    
    id_contact = 0
    while len(phone_contacts)!=0:
        try:
            flag_delete = click_delete_option(id_contact)

            if flag_delete:
                confirm_delete_contact()
                possible_emerged_erro()
                phone_contacts = wait_list_contacts()
            else:
                id_contact += 1
            if len(phone_contacts)==1 and id_contact!= 0:
                break        
...
def automatization_second_web_site(dict_credentials, stop_event):
    ...
    if not os.path.isfile('check_points/last_row.json'):
        ...
        try:            
            ...
            user_input =input("Do you want to continue from the previous point? Type ‘y’ to continue from the previous point, or any other key to restart.")
            if user_input =='y':
                row_number
        except:
            # last_row = 0
    ...
    while row_number <= len(df): #and not stop_event:
        ...
        try:
            ...
            try:
                ...
                ignoreanswer.click()
                print("The issue is caused by the input message interfering.")
            except:
                print("Other issue")
    if stop_event:
        print("Process stoped")
    print("File Processed")
    user_input = input("Do you want to delete all the chats and contacts? Type ‘y’ to delete, or any other key to continue. ")
    if user_input =='y':
        delete_chats()
        delete_contacts()
    print("Process end")    
